          ...
          <div class="card rounded-0 border-0 {{ $userdata->package_id === $package->id ? 'shadow' : '' }} plans">
               <div class="card-body">
                    ...
                    <div class="text-center">
                         <button type="button" id="" class="btn btn-sm {{ $userdata->package_id === $package->id ? 'btn-secondary disabled' : 'btn-primary'}} rounded-0 add-to-cart" data-package-id="{{ $package->id }}" data-package-price="{{ $package->price }}">Add Cart</button>
                    </div>
               </div>
          </div>
     </div>
